...
The architecture defines the Daedalus API as the C# ASP.NET Core control plane responsible for routing job submission, job status visibility, scheduler configuration management, cancellation intake, and report serving. No specification currently defines the precise endpoint contracts, request and response models, validation behavior, error response format, idempotency expectations, or observability obligations governing the API's external-facing behavior.
...
|---|---|---|
| **API** (SPEC-008) | Request receipt; structural and fast domain validation; routing problem persistence; job creation; queue publication; job status visibility; scheduler configuration management; cancellation request intake; report metadata and file serving; `job.submit` span emission; error response production | Solver execution; backend selection; workload feature extraction; quality evaluation; report generation; evidence record persistence; Worker execution behavior |
...
5. Create the job record in PostgreSQL with `job_id`, `problem_id`, `scheduler_config_id`, `status = Pending`, and `submitted_at` timestamp.
...
**Description:**
Project DAEDALUS uses two public identifiers per job submission. Both are system-generated UUIDs. Internal execution identifiers are not exposed in any API response.
...
| `status` | string | Always | Current lifecycle state (FR-9) |
...
| `solver_outcome` | string | When `status = Completed` | The solver outcome code from the solver run record; absent for `Failed` jobs that did not reach solver invocation |
...
- A `Completed` job always includes `solver_outcome` when one was recorded; it may be absent if the job failed before solver invocation
...
| `Completed` | Worker completed its execution lifecycle and produced an evidence record. This state does not indicate solver success; solver outcomes of `Succeeded`, `Infeasible`, `Timeout`, `Cancelled`, and `Failed` all produce a `Completed` job. | Yes |
| `Failed` | Worker could not complete its orchestration responsibilities. No complete evidence record was produced. | Yes |

**State visibility constraint:** The API reads job state from the PostgreSQL job record written by the Worker (SPEC-005 FR-2). The API does not infer or invent state transitions.

**Caller guidance:**
Callers must not assume that `Completed` means the solver found a valid solution. Callers should inspect `solver_outcome` to determine the optimization result and consult the evidence report for full decision context.
...
| `mode_parameters` | object | Conditional | Required parameters for the selected mode; see SPEC-003 FR-6 Parameter Schemas |
...
- Excess `mode_parameters` fields for non-parameterized modes are accepted and ignored
...
| `request_id` | Always | A UUID generated per request; correlates with `api.request_id` OTel span attribute |
...
The `job.submit` span context must be propagated into the job message (or stored with the job record) so that the Worker's `job.consume` span can be linked to the originating submission span. The specific correlation mechanism (W3C TraceContext in the AMQP message headers, or a stored correlation ID on the job record) is an implementation planning concern.
...
8. No authentication or authorization is required at MVP scope. All API endpoints are accessible without credentials. Multi-tenant security is deferred (README).
...
| Job record | PostgreSQL | `job_id`, `problem_id`, `scheduler_config_id`, `status = Pending`, `submitted_at` | Initial creation; Worker updates status |
...
| Configuration | Yes — API manages scheduler configuration records; seeds default configuration at startup |
...
Multi-tenant security is deferred (README). All API endpoints are accessible without credentials at MVP scope. This is an accepted risk for a local development and demonstration environment. Authentication and authorization must be added before any production deployment or exposure to untrusted networks.
...
**Owner:** Implementation planning; OpenTelemetry SDK capabilities for both .NET and C++ must be assessed (ADR-006).
...
- [x] Scheduler configuration management is defined (FR-10)
