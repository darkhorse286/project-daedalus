...
10. The API must emit CORS response headers (`Access-Control-Allow-Origin` and related preflight headers) permitting requests from the browser client origin. The browser client origin is `http://localhost:3000` in the standard Docker Compose configuration per ADR-014 Decision 5. CORS configuration is a middleware and deployment concern; it does not modify any endpoint contract or introduce new endpoints.
...
- **ADR-014 (Browser Client Architecture, Accepted)**: ADR-014 Decision 5 establishes the browser client origin as `http://localhost:3000`. The API must be configured to emit CORS response headers permitting requests from this origin. This is captured in Constraint 10. SPEC-021 Architectural Impact and SPEC-022 Architectural Impact both noted this requirement; ADR-014 Decision 5 is now the authoritative resolution.
