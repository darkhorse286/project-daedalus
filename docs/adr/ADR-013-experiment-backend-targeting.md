...
- FR-1 (Scheduler Responsibility Scope): Amend to describe both execution paths. In the standard path, the Scheduler evaluates all registered backends, scores eligible candidates, and selects the highest-scoring one. In the targeted path (`backend_id` present), the Scheduler evaluates the specified backend only, bypasses scoring, and confirms the backend deterministically if eligible. Both paths produce a decision record and return it to the caller.
...
- FR-10 (Decision Record): Add `selection_mode` field. Valid values: `policy_selected` (standard path, Scheduler selected by objective function and eligibility), `explicitly_targeted` (targeted path, caller specified the backend). The `selection_mode` field must be persisted in the decision record and must appear in all decision record outputs consumed by the evidence report (SPEC-009) and the Evidence Log (SPEC-006). Specify `candidate_scores` and `confidence_score` semantics for the targeted path: `candidate_scores` is empty (scoring bypassed); `confidence_score` is `0.0`. The `selection_mode = explicitly_targeted` field provides context distinguishing this `0.0` from the standard-path single-eligible-backend case. Specify `rejection_reasons` semantics for the targeted path: if the targeted backend fails Phase 1 or Phase 2 eligibility checking, `rejection_reasons` contains the targeted backend's rejection reason code; if the targeted backend is eligible, `rejection_reasons` is empty. Non-targeted backends are not evaluated and produce no `rejection_reasons` entries. The `selection_mode = explicitly_targeted` field communicates to evidence report consumers that the absence of non-targeted backend rejection reasons reflects an explicitly constrained evaluation, not a policy-based multi-backend assessment.
